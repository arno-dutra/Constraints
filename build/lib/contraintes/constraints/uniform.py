...
class UniformConstraintsGenerator(ConstraintsObject):
    ...
    def generate_must_link(self, nb_constraints, with_replacement=False, based_on_y=True, no_tautology=False):
        ...
        points_to_constraint = list(set(np.random.choice(self.ids, nb_constraints, replace=with_replacement)))
        ...
        if based_on_y:
            ...
            for c_a in points_to_constraint:
                ...
                c_b = np.random.choice(np.where(self.y == self.y[c_a])[0], 1)
                if no_tautology:
                    while c_a == c_b:
                        c_b = np.random.choice(np.where(self.y == self.y[c_a])[0], 1)

                self.add_must_link(int(c_a), int(c_b))
        else:
            for c_a in points_to_constraint:

                if no_tautology:
                    c_b = np.random.choice(points_to_constraint - {c_a}, 1)
                else:
                    c_b = np.random.choice(points_to_constraint, 1)
    ...
    def generate_cannot_link(self, nb_constraints, with_replacement=False, based_on_y=True):
        ...
        if based_on_y:
            ...
            for c_a in points_to_constraint:
                ...
                self.add_cannot_link(int(c_a), int(c_b))
        else:
            for c_a in points_to_constraint:
